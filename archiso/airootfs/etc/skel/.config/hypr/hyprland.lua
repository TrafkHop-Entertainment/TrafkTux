...
-- TrafkTuxLauncher ersetzt rofi fuer AppLauncher/PowerMenu komplett:
-- ein eigenstaendiges GTK3+gtk-layer-shell-Programm, kein rofi-
-- Subprozess mehr, kein -theme/-kb-*/-monitor-Kommandozeilenkram
-- mehr noetig (Bubble-Rendering, Menuebaum, Tastatur- und Maus-
-- Navigation stecken jetzt alle direkt im Binary). Binary liegt
-- nach dem gleichen Muster wie TrafkTuxBar unter
-- ~/.config/TrafkTuxLauncher/TrafkTuxLauncher.
local launcher_cmd = "$HOME/.config/TrafkTuxLauncher/TrafkTuxLauncher --show AppLauncher"
local powermenu_cmd = "$HOME/.config/TrafkTuxLauncher/TrafkTuxLauncher --show PowerMenu"
-- ALT (bis hierher lief das über rofi im Script-Mode, siehe
-- RofiTrafkBubbleMenus.c) - als Referenz stehen gelassen, falls
-- mal nachvollzogen werden muss, wie das vorher funktioniert hat:
...
hl.on("hyprland.start", function()
    ...
    -- Assets liegen jetzt unter ~/.config/Assets/ (gleiche Dateinamen,
    -- neuer Ordner - vorher ~/.config/rofi/assets/).
    -- BUGFIX: hier wurden Glow1/Glow2 bisher als reine Kopien von
    -- TrafkBubble1/2 (normal) nach /tmp kopiert - das hat bei JEDEM
    -- Hyprland-Start die echten Glow-Bilder in /tmp überschrieben,
    -- weshalb die ausgewählte Blase optisch nie anders aussah als die
    -- unselektierte. Jetzt werden die echten TrafkBubbleGlow1/2.png
    -- kopiert.
    hl.exec_cmd("cp ~/.config/Assets/bubble-normal.png /tmp/bubble-normal.png")
    hl.exec_cmd("cp ~/.config/Assets/bubble-selected.png /tmp/bubble-selected.png")
    hl.exec_cmd("cp ~/.config/Assets/TrafkBubble1.png /tmp/TrafkBubble1.png")
    hl.exec_cmd("cp ~/.config/Assets/TrafkBubble2.png /tmp/TrafkBubble2.png")
    hl.exec_cmd("cp ~/.config/Assets/TrafkBubbleGlow1.png /tmp/TrafkBubbleGlow1.png")
    hl.exec_cmd("cp ~/.config/Assets/TrafkBubbleGlow2.png /tmp/TrafkBubbleGlow2.png")
    hl.exec_cmd("cp ~/.config/Assets/Right.png /tmp/Right.png")
    hl.exec_cmd("cp ~/.config/Assets/Left.png /tmp/Left.png")
    ...
    -- TrafkTuxLauncher-Daemon: haelt AppLauncher- und PowerMenu-Fenster
    -- fertig gebaut & versteckt im Hintergrund, launcher_cmd/powermenu_cmd
    -- oben schicken nur noch ein "zeig dich"-Signal per Unix-Socket -
    -- kein GTK-Neustart mehr pro Tastendruck, dadurch quasi instant.
    -- Muss NACH den cp-Zeilen oben stehen (lädt /tmp/Trafk*.png beim
    ...
    hl.exec_cmd(
        "systemctl --user start WidgetsDaemon.service FocusFixDaemon.service ScreenRotationDaemon.service SoundCenter.service"
    )
    hl.exec_cmd("sudo systemctl restart trafktux-grub-theme-sync.service")
...
--
-- HINWEIS (gemergt): frueher gab es hier ZWEI separate
-- hl.config({ plugin = { hyprgrass = {...} } })-Aufrufe mit
-- ueberschneidenden, teils widerspruechlichen Keys (sensitivity
-- wurde zweimal mit unterschiedlichen Werten gesetzt, edge_margin
-- und long_press_delay standen nur je in einem der beiden Bloecke).
-- Jetzt EIN Block mit allen vier Keys, nichts wird mehr
-- unvorhersehbar ueberschrieben/verloren.
-- long_press_delay testweise von 400 auf 700ms erhoeht - falls
-- kurze Taps weiterhin sofort als Rechtsklick durchgehen, hier
-- weiter hochdrehen.
...
-- hypr-kinetic-scroll: kompositor-seitiges kinetisches (inertiales)
-- Touchpad-Scrollen, https://github.com/savonovv/hypr-kinetic-scroll
-- Einmalig manuell einrichten (macht die Lua-Config nicht selbst):
...
if hl.plugin and hl.plugin.kinetic_scroll then
    ...
    -- Beispiel fuer Per-App-Regeln ueber die offizielle Lua-API
    -- (statt disabled_classes im INI-Stil):
    -- hl.plugin.kinetic_scroll.disable("org.telegram.desktop")
    -- hl.plugin.kinetic_scroll.disable("steam")
else
    hl.exec_cmd(
        "bash -c \"notify-send 'Hyprland' 'hypr-kinetic-scroll nicht geladen (siehe: hyprctl plugin list)' -u critical || true\""
    )
end
...
hl.bind(
    mainMod .. " + CTRL + code:49",
    hl.dsp.exec_cmd("hyprctl dispatch 'hl.dsp.exec_cmd(\"~/.config/hypr/PictureInPicture.sh\")'")
)
...
hl.bind(
    mainMod .. " + H",
    hl.dsp.exec_cmd("bash ~/.config/waybar/Scripts/HideAll.sh toggle &"),
    { description = "Alle Fenster verstecken/wiederherstellen" }
)
...
local function get_monitor_in_direction(current_monitor, direction)
    ...
    for _, m in ipairs(monitors) do
        if direction == "right" and m.x > current_monitor.x then
            if not best or m.x < best.x then
                best = m
            end
...
local function scrolling_focus(direction)
    ...
    local function get_x(w)
        if type(w.at) == "table" and type(w.at.x) == "number" then
            return w.at.x
        end
        if type(w.at) == "table" and type(w.at[1]) == "number" then
            return w.at[1]
        end
        if type(w.position) == "table" and type(w.position.x) == "number" then
            return w.position.x
        end
...
local function get_pos(w)
    if type(w.at) == "table" and type(w.at.x) == "number" then
        return w.at.x, w.at.y
    end
    if type(w.at) == "table" and type(w.at[1]) == "number" then
        return w.at[1], w.at[2]
    end
    if type(w.position) == "table" and type(w.position.x) == "number" then
...
local function floating_focus_direction(active, direction)
    ...
    if #candidates == 0 then
        debug_float_nav(
            "Richtung="
                .. direction
                .. ": 0 floatende Kandidaten auf diesem Workspace (inkl. gepinnter) - falle auf natives movefocus zurück"
        )
        return false
    end

    local function matches(dx, dy)
        if spec.dx < 0 and dx >= 0 then
            return false
        end
        if spec.dx > 0 and dx <= 0 then
            return false
        end
        if spec.dy < 0 and dy >= 0 then
            return false
        end
        if spec.dy > 0 and dy <= 0 then
            return false
    ...
    debug_float_nav(
        "Richtung="
            ...
            .. ": "
            .. #candidates
            .. " Kandidaten, aber KEINER hat gepasst (auch Wraparound leer) - falle auf natives movefocus zurück"
    )
    return false
end
...
local function focusOrCycle(direction)
    return function()
        ...
        local active = hl.get_active_window()
        debug_float_nav(
            "focusOrCycle Start: layout="
                .. tostring(layout)
                .. " skipFloatFirst="
                .. tostring(skipFloatFirst)
                .. " floatingMode="
                .. tostring(is_floating_mode_active())
                .. " active.floating="
                .. tostring(active and active.floating)
        )
        ...
        elseif skipFloatFirst then
            debug_float_nav(
                "Richtung="
                    .. direction
                    .. ": floating_focus_direction uebersprungen, weil layout="
                    .. tostring(layout)
                    .. " (monocle/scrolling)"
            )
        elseif not (active and active.floating) then
            debug_float_nav(
                "Richtung="
                    .. direction
                    .. ": aktives Fenster ist NICHT floatend (oder gar nicht ermittelbar) -> floating_focus_direction wird nicht versucht"
            )
        end
        ...
        debug_float_nav(
            "Richtung=" .. direction .. ": lande beim nativen hl.dsp.focus({direction=...}) (letzter Fallback)"
        )
...
if hl.plugin and hl.plugin.hyprgrass then
    ...
    -- 3-Finger-Wisch links/rechts: Workspace-Wechsel mit Wraparound,
    -- analog zu SHIFT+q / SHIFT+e. Logik 1:1 kopiert, damit das Verhalten
    -- identisch ist und sich beide Eingabewege gleich anfühlen.
    ...
    hl.plugin.hyprgrass.bind({
        ...
        action = workspaceCycle("left"),
    })

    hl.plugin.hyprgrass.bind({
        pattern = { kind = "swipe", fingers = 3, direction = "r" },
        action = workspaceCycle("right"),
    })
end
...
hl.bind(
    mainMod .. " + SHIFT + Print",
    hl.dsp.exec_cmd(
        'bash -c "mkdir -p ~/Screenshots && grim -g \\"$(slurp -d)\\" ~/Pictures/$(date +%Y%m%d_%H%M%S).png"'
    )
)
...
if hl.plugin.hyprglass then
    ...
    hg.layer("TrafkTuxLauncher", { exclude = true })
    ...
    hl.define_submap("hyprexpo", function()
        ...
        for i = 1, 9 do
            hl.bind(tostring(i), function()
                hl.plugin.hyprexpo.kb_selecti(i)
            end)
        end
...
local function clamp_floating_window(win)
    ...
    if is_size_clamp_excluded(win) then
        return
    end
    local mon = win.monitor
    local scale = mon.scale or 1
...
hl.bind(mainMod .. " + SHIFT + s", function()
    local win = hl.get_active_window()
    if win then
        hl.dispatch(hl.dsp.window.resize({ x = win.size.x, y = win.size.y - 50 }))
    end
end, { repeating = true })
...
hl.bind(mainMod .. " + SHIFT + a", function()
    local win = hl.get_active_window()
    if win then
        hl.dispatch(hl.dsp.window.resize({ x = win.size.x - 50, y = win.size.y }))
    end
end, { repeating = true })
