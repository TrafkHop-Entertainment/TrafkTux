...
# Separates Flag-File, das hyprland.lua bei JEDEM window.open live (ohne
# Reload) ausliest, um neue Fenster zu floaten, solange "floating" als
# Layout aktiv ist. Bewusst getrennt von STATE_FILE: STATE_FILE ist nur
# fürs zugrundeliegende Tiling-Layout (general.layout) und wird von
# "floating" absichtlich NICHT überschrieben (siehe Kommentar oben).
FLOATING_FLAG="$STATE_DIR/floating_mode_active"
mkdir -p "$STATE_DIR"
...
# Jede echte Layout-Wahl (master/dwindle/scroller/bigscreen) beendet den
# Floating-"Modus" implizit - das Flag wird deshalb unconditional VOR dem
# case gelöscht. Der floating)-Case setzt es danach gezielt neu.
rm -f "$FLOATING_FLAG"
...
#
# Ausnahmen: gepinnte Fenster (.pinned != true) und dunst (.class != "Dunst",
# case-insensitive) - dunst ist selbst floating/nicht gepinnt, würde also sonst
# beim Layout-Wechsel mit ins Tiling gezogen und flackert/verzerrt sichtbar,
# wenn gerade eine Notification offen ist.
unfloat_current_workspace() {
    CURRENT_WS=$(hyprctl activeworkspace -j | jq '.id')
    hyprctl clients -j | jq -r ".[] | select(.workspace.id == $CURRENT_WS and .floating == true and .pinned != true and (.class | ascii_downcase != \"dunst\")) | .address" | while read -r addr; do
...
case "$ACTION" in
    master|dwindle)
        ...
        bash "$HOME/.config/hypr/SoundCenter.sh" LayoutSwitch &
        ;;
    scroller|scrolling)
        unfloat_current_workspace
        ...
        ;;
    floating)
        # Flag setzen (wieder aufgehoben durch das "rm -f" oben, sobald
        # master/dwindle/scroller/bigscreen gewählt wird) - hyprland.lua
        # liest das bei jedem window.open live und floatet neue Fenster.
        touch "$FLOATING_FLAG"
        CURRENT_WS=$(hyprctl activeworkspace -j | jq '.id')
        hyprctl clients -j | jq -r ".[] | select(.workspace.id == $CURRENT_WS and .floating == false) | .address" | while read -r addr; do
            hyprctl eval "hl.dispatch(hl.dsp.window.float({ action = 'toggle', window = 'address:$addr' }))"
            # Direkt danach die 35%-Max-Größe erzwingen (global exponierte
            # Funktion aus hyprland.lua, siehe deren Kommentar bei
            # "hlClampFloatingWindow") - deckt "beim Layout-Wechsel auf
            # floating" ab, ohne auf ein moeglicherweise unzuverlaessiges
            # window.update_rules-Event von aussen angewiesen zu sein.
            hyprctl eval "hlClampFloatingWindow(hl.get_window('address:$addr'))"
        done
        bash "$HOME/.config/hypr/SoundCenter.sh" LayoutSwitch &
        ;;
    bigscreen|monocle)
