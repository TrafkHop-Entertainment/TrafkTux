...
# WICHTIG, anders als bei SoundCenter.sh: reine Parameter-Expansion
# reicht hier NICHT. make_cursor() macht weiter unten intern ein
# "(cd \"$WORK\" && xcursorgen ... \"$OUT/cursors/$shape\")" - wäre
# $OUT relativ (z.B. "./Output/..."), würde es nach dem cd auf einmal
# relativ zum NEUEN Arbeitsverzeichnis aufgelöst und auf den falschen
# Ort zeigen (genau das, was gerade passiert ist). Der pwd-Subshell
# kostet bei einem einmalig von Hand gestarteten Build-Script (Laufzeit:
# mehrere Sekunden) nichts Spürbares - anders als beim latenzkritischen
# SoundCenter.sh.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

PNG="$SCRIPT_DIR/TrafkTuxCursorPNGs"
WORK="$SCRIPT_DIR/XcursorBuildtemp"
...
# Dichter gestuft als vorher (gleicher Grund wie im Hyprcursor-Build-
# Script: 35 und 45 waren die einzigen "sauberen" Groessen, dazwischen/
# darueber hinaus musste interpoliert werden, was am Cursor sichtbar
# abgeschnitten wirkte). 35 und 45 bleiben erhalten.
SIZES=(24 28 32 35 38 42 45 48 52 56 60 64)

# Selbes Problem wie im Hyprcursor-Build-Script: pointer.png ist hochkant,
# daher füllte das Artwork nach "-resize NxN" auf ein Quadrat immer exakt
# die volle Zielhöhe (0px Puffer unten). ARTWORK_FRAC erzwingt einen
# echten Rand auf allen Seiten.
ARTWORK_FRAC=88

rm -rf "$WORK"
mkdir -p "$WORK/imgs"

# Loescht nur Xcursor-Spezifisches im Zielordner
...
render_size() {
  ...
  [ "$inner" -lt 1 ] && inner=1
  magick "$PNG/$img" -resize ${inner}x${inner} -gravity North -background none -extent ${size}x${size} "$out"
}
...
echo "Xcursor-Theme fertig generiert unter: $OUT"
