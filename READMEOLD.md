...
**Gaming**  
We have gamescope and wine preinstalled! We also have a custom script, that puts any launched wine game into gamescope, so that all games, be it Crazy Chicken, Austin Cooper S Racing, Road Rash, other old games, can run in a scalable windowed mode the same way any other program can!  
...
- [hytale-launcher-bin*(aur)*](https://aur.archlinux.org/packages/hytale-launcher-bin "https://aur.archlinux.org/packages/hytale-launcher-bin")  
**Flatpak:**  
...
- [azaharplus-appimage](https://aur.archlinux.org/pkgbase/azaharplus-appimage "https://aur.archlinux.org/pkgbase/azaharplus-appimage")  
...
- when firefox is in fullscreen, the keyboard inputs most of the time are transferred to another window (thunar in our case always) but firefox is still the active window and the mouse can be used. if fullscreen is left, the firefox window goes (in scroller) to the left side, not the right. this means the right side does not have a window, wich should not be possible.
  to get keyboard back to firefox you have to change the active window to another window and back.
  this is a firefox issue and we cannot fix it!
- xfce4desktop sometimes crashes... this does not happen as quickly as swaync but can happen. normally it crashes sometimes after a few hours but is very inconsistant and does not happen often.
